...
void main() {
  ...
  setUp(() async {
    ...
    await File(p.join(workspace.path, 'Cards', 'card.md')).create(
      recursive: true,
    );
    await File(
  ...
  test('restore rejects a backup with a mismatched manifest checksum',
      () async {
    final outputDir = Directory(p.join(tempDir.path, 'Backups'));
    final backupPath = await BackupService.createBackup(
      outputDirectory: outputDir.path,
  ...
  test('automatic backup skips a second run within twenty-four hours',
      () async {
  ...
  test('automatic backup retention keeps the newest seven daily snapshots',
      () async {
    ...
    final files = await backupDir
        .list()
        .where((entity) =>
            entity is File && p.basename(entity.path).startsWith('memex_auto'))
        .toList();
    ...
    final safetyFiles = await backupDir
        ...
        .where((entity) =>
            ...
            p.basename(entity.path).startsWith(
                  'memex_safety_before_storage_switch',
                ))
        .toList();
